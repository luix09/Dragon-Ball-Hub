...
class UserManager extends StateNotifier<UserManagerStatus> {
  ...
  Future<UserManagerStatus> getCurrentUserData() async {
    try {
      final uid = userAuth.authHelper.auth.currentUser!.uid;
      DocumentSnapshot? data = await firestoreHelper.getCurrentUserData(uid);
      UserDataModel userData = UserDataModel.fromMap(data!.data()!);
      state = UserManagerStatus.GotUser;
    } catch (e) {
      print("Exception in deleteUser UserManager: $e");
      state = UserManagerStatus.undefined;
    }
